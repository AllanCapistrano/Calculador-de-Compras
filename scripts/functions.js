...
function showItens(num) {
  if (tab.children[num] != null) {
    tab.children[num].textContent = `${name[num].value}: ${valores[num].toLocaleString(
      'pt-BR',
      {
    ...
    item.text = `${name[num].value}: ${valores[num].toLocaleString(
      'pt-BR',
      {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
...
function showTotal(num) {
  if (tab.children[num] != null) {
    tab.children[num].textContent = `Total: ${total.toLocaleString(
      'pt-BR',
      {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    ...
    let item = document.createElement('option');
    item.text = `Total: ${total.toLocaleString(
      'pt-BR',
      {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
